...
export async function POST(
  ...
  try {
    const decoded = (await authenticate(request)) as DecodedToken | null;
    if (!decoded)
    ...
    const res: any = await DatabaseService.query(sql, [
      userId,
      documentId,
      userId,
    ]);
    const affectedRows =
      typeof res?.affectedRows === "number"
        ? res.affectedRows
        : Array.isArray(res) && res[0]?.affectedRows
        ? res[0].affectedRows
        : 0;

    if (affectedRows > 0) {
      return NextResponse.json(
        { message: "Lock acquired", locked_by: userId },
        { status: 200 }
      );
    }

    // Fetch current locker if lock not acquired
...
export async function DELETE(
  ...
  try {
    ...
    // ✅ Safely check admin property with type support
